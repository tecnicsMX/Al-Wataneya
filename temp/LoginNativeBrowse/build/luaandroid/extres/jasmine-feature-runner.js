/* globals ddescribe:false */
    ...
    function jasmineFeatureRunner(features, steps){
       return function(feature){
        ...
        describe(feature.description, function(){
         ...
         /*   describe('\nFeature: ' + feature.description, function(){
                ...
                it(scenario.description, getScenarioRunner(scenario));

                 function getScenarioRunner(scenario){
                    var steps = scenario.beforeSteps.map(function(definition){
                    return {
                        description : '',
                        step  :async function(scenarioContext){
                        await definition.call(scenarioContext);
                        }
                    };
                    })
                    .concat(
                    scenario.steps.map(function(scenarioStep){
                        ...
                        step : async function(scenarioContext){
                            var step =await getStep(scenarioStep);
                            if (step && missingSteps.length === 0 && ambiguousSteps.length === 0){
                               await step(scenarioContext);
                            }
                        }
                        };
                    })
                    ...
                    var scenarioExecuter =async function(){
                    var scenarioContext = {
                            when :async function(description){
                                var step = await getStep({
                                    description : description,
                                    arguments : Array.prototype.slice(arguments, 1)
                                });
                                if (step !== noOp){
                                  return  await step(scenarioContext);
                                }
                                else{
                            ...
                            given :async function(){
                              return await this.when.apply(this, arguments);
                            },
                            then : async function(){
                              return  await this.when.apply(this, arguments);
                            },
                        ...
                        executeNextStep =async  function(){
                            ...
                            if (currentStep + 1 < steps.length){
                              ...
                              executingStep = true;
                             await steps[currentStep].step(scenarioContext)
                              executingStep = false;
                            ...
                            //  done();//as part of async/await changes
                          }
                ...
                if (matchingSteps.length === 1){
                    return async function(scenarioContext){
                        ...
                        try{
                         return  await matchingSteps[0].definition.apply(scenarioContext, matchingSteps[0].arguments);
                        }
                        catch(e){
